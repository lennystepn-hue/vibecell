  ...
  <div class="min-h-screen text-fg-primary" style="background: var(--bg-body-to)">
    ...
    <section class="max-w-3xl mx-auto px-4 sm:px-6 pt-[88px] sm:pt-[112px] pb-8 sm:pb-12 text-center">
      ...
      <h1 class="font-semibold mb-4" style="font-size: clamp(2rem, 4vw, 3rem); letter-spacing: -0.04em; color: var(--fg-primary); line-height: 1.1">
        ...
        <span style="color: var(--signal-green)">No funnel games.</span>
      </h1>
    ...
    >
      ...
      >
        ...
        <span style="font-size: 11px; color: var(--fg-muted); letter-spacing: 0.02em">
    ...
    <section class="max-w-md mx-auto px-4 sm:px-6 pb-16 sm:pb-24">
      <div class="rounded-xl p-8 flex flex-col relative overflow-hidden"
        ...
        <template v-if="billingCycle === 'monthly'">
          <div class="flex items-end gap-2 mb-1">
            <span class="font-bold" style="font-size: 4rem; color: var(--fg-primary); letter-spacing: -0.05em; line-height: 1">€8.99</span>
            <span class="mb-3" style="font-size: 14px; color: var(--fg-muted)">/ month</span>
          </div>
          <p class="mb-8" style="font-size: 12px; color: var(--fg-subtle)">
        ...
        <template v-else>
          <div class="flex items-baseline gap-2 mb-1 flex-wrap">
            ...
            <span style="font-size: 14px; color: var(--fg-muted)">/ year</span>
            <!-- Was-price as a sub-typographic note, not a co-equal sibling -->
            <span
              v-if="launch.active"
              class="font-mono ml-1"
              style="font-size: 12px; color: var(--fg-subtle); text-decoration: line-through; text-decoration-thickness: 1px; letter-spacing: 0"
            >€99.99</span>
          </div>
          <p v-if="launch.active" class="mb-1 font-mono" style="font-size: 10px; color: var(--signal-amber); letter-spacing: 0.12em; text-transform: uppercase">
        ...
        <ul class="space-y-3 mb-10">
          <li v-for="f in features" :key="f.text" class="flex items-start gap-3" style="font-size: 13px; color: var(--fg-muted)">
            <svg viewBox="0 0 16 16" fill="none" style="width:14px;height:14px;flex-shrink:0;margin-top:2px">
              <circle cx="8" cy="8" r="7" stroke="rgb(var(--signal-green-rgb) / 0.5)" stroke-width="1"/>
              <path d="M5 8l2 2 4-4" stroke="var(--signal-green)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            <span>
              <strong v-if="f.emphasis" style="color: var(--fg-primary)">{{ f.text }}</strong>
        ...
        <p class="text-center mt-2.5 font-mono" style="font-size: 10px; color: var(--fg-subtle)">
      ...
      <p class="text-center mt-6 font-mono" style="font-size: 11px; color: var(--fg-subtle)">
    ...
    <section class="max-w-2xl mx-auto px-4 sm:px-6 pb-16 sm:pb-28">
      ...
      <div class="space-y-2">
        <div
          ...
          style="border: 1px solid rgb(var(--border-rgb) / 0.1)">
          <button
            ...
            @click="toggleFaq(i)">
            <span class="font-semibold pr-4" style="font-size: 13px; color: var(--fg-body); letter-spacing: -0.01em">
              {{ faq.q }}
            </span>
            <svg viewBox="0 0 16 16" fill="none" class="flex-shrink-0 transition-transform duration-200"
              ...
              <path d="M4 6l4 4 4-4" stroke="var(--fg-subtle)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </button>
          <div
            v-if="openFaq === i"
            class="px-5 py-4"
            style="background: rgb(var(--bg-surface-rgb) / 0.2); border-top: 1px solid rgb(var(--border-rgb) / 0.07); font-size: 13px; color: var(--fg-muted); line-height: 1.65">
            {{ faq.a }}
          </div>
        </div>
      </div>
    ...
    <section class="text-center py-16 sm:py-20 px-4 sm:px-6"
      style="background: rgb(var(--signal-green-rgb) / 0.03); border-top: 1px solid rgb(var(--signal-green-rgb) / 0.1)">
      <h2 class="font-semibold mb-4" style="font-size: clamp(1.4rem, 2.5vw, 2rem); letter-spacing: -0.03em; color: var(--fg-primary)">
        Ready when you are.
      </h2>
      <p class="mb-8" style="font-size: 14px; color: var(--fg-muted)">
    ...
    <footer class="px-6 py-8" style="border-top: 1px solid var(--border-subtle)">
      <div class="max-w-5xl mx-auto flex items-center justify-between" style="font-size: 12px; color: var(--fg-subtle)">
